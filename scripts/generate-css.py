...
def render_grid():
    """Optional Tailwind spacing grid. Base-independent, no strike data needed.

    One source pixel drives --spacing, so every spacing utility (p-* m-* gap-*
    space-* inset-* size-*) is an integer number of source pixels and zooms with
    html { font-size } exactly like the scale. --qs-px is the unit itself, kept
    in :root so it exists for hand-rolled lengths even when Tailwind tree-shakes.
    Border widths aren't spacing-derived, so the common ones are pinned here too.
    """
    rem = rem_size(1)  # one source pixel = 1/16rem, same divisor as the scale
    return (
        "/* quanta-strike — generated by scripts/generate-css.py. Do not edit by hand.\n"
        " *\n"
        " * Optional spacing grid. --spacing becomes one source pixel, so p-* m-*\n"
        " * gap-* space-* inset-* size-* land on the strike grid and zoom with\n"
        " * html { font-size } like the scale. Base-independent; pairs with any\n"
        " * base-N. Reuse --qs-px for hand-rolled lengths: border-width: var(--qs-px).\n"
        " *\n"
        " *   @import \"tailwindcss\";\n"
        ...
        "  --spacing: var(--qs-px);\n"
        "}\n"
        "\n/* Border widths aren't spacing-derived; pin the common ones to the grid too. */\n"
        "@layer utilities {\n"
